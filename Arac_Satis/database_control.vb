...
Public Class Database_Control
    ...
    Public Function Connect_Data(ByVal database_full_path As String, ByVal datatable_name As String, ByVal dataGrid As DataGridView) As DataTable
        Dim cs As String = "URI=file:" & database_full_path
        Dim conn = New SQLiteConnection(cs)
        Dim adapter = New SQLiteDataAdapter("SELECT * FROM " + datatable_name, conn)
    ...
    Public Function Returnable_satis(ByVal database_full_path As String, ByVal tarih As String) As DataTable
        Dim dgrid As DataGrid = New DataGrid
        Dim dtable As DataTable = New DataTable
        Dim satislar As List(Of String) = New List(Of String)
        Dim cs As String = "URI=file:" & database_full_path
        Try
            Dim data_connection = New SQLiteConnection(cs)
            data_connection.Open()
            Dim stm As String = "SELECT * FROM satislar WHERE satis_tarihi LIKE '%" & tarih & "%'"
            Dim cmd = New SQLiteCommand(stm, data_connection)
            ...
            'While reader.Read()
            '    ' tc_no değerini alın
            '    Dim tcNo As String = reader("tc_no").ToString()
            '    Dim tamAd As String = reader("tam_ad").ToString()
            '    Dim telNo As String = reader("tel_no").ToString()
            '    Dim ehliyetNo As String = reader("ehliyet_no").ToString()
            '    Dim ehliyetTarihi As String = reader("ehliyet_tarihi").ToString()
            '    Dim marka As String = reader("marka").ToString()
            '    Dim model As String = reader("model").ToString()
            '    Dim seriNo As String = reader("seri_no").ToString()
            '    Dim renk As String = reader("renk").ToString()
            '    Dim plaka As String = reader("plaka").ToString()
            '    Dim fiyat As String = reader("fiyat").ToString()
            '    Dim satisTarihi As String = reader("satis_tarihi").ToString()
    ...
    Public Function Returnable_Run_Command(ByVal database_full_path As String, ByVal command As String, ByVal column_name As String) As List(Of String)
        Dim tcno As List(Of String) = New List(Of String)
        Dim cs As String = "URI=file:" & database_full_path
        Try
            Dim data_connection = New SQLiteConnection(cs)
            data_connection.Open()
            Dim stm As String = command
            Dim cmd = New SQLiteCommand(stm, data_connection)
            reader = cmd.ExecuteReader()
            While reader.Read()
                ' tc_no değerini alın
                tcno.Add(reader(column_name).ToString())
    ...
    Public Function Returnable_last10(ByVal database_full_path As String) As DataTable
        Dim dgrid As DataGrid = New DataGrid
        Dim dtable As DataTable = New DataTable
        Dim satislar As List(Of String) = New List(Of String)
        Dim cs As String = "URI=file:" & database_full_path
        Try
            Dim data_connection = New SQLiteConnection(cs)
            data_connection.Open()
            Dim stm As String = "SELECT * FROM satislar ORDER BY id DESC LIMIT 10"
            Dim cmd = New SQLiteCommand(stm, data_connection)
            reader = cmd.ExecuteReader()
            dtable.Load(reader)
            dgrid.DataSource = dtable
